...
export class ImagePresenterComponent implements OnInit {
    ...
    private isValidFiles(files: File[]){
    ...
    private isValidFileExtension(files: File[]){
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            var exists = extensions.indexOf(ext) >= 0;
            if (!exists) {
    ...
    saveFiles(files: File[]){
        ...
        if (files.length > 0) {
              ...
              this.fileService.upload(formData, this.company)
                  .subscribe(
                  success => {
